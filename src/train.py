...
class ModelArguments:
    model_name_or_path: Optional[str] = field(default="Qwen1.5-4B-Chat-clip-vit-large-patch14-336/model")
...
def load_model_processor(modelargs: ModelArguments):
    ...
    if modelargs.train_type == "use_lora":
        ...
        config = LoraConfig(
            ...
            bias="none",
            task_type="CAUSAL_LM",
            modules_to_save=["multi_modal_projector"], # List of modules apart from adapter layers to be set as trainable and saved in the final checkpoint.
        )
        model = get_peft_model(model, config)

    elif modelargs.train_type == "none":
        # train vision, mmprojector, llm
